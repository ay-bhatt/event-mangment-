...
import { getVolunteers, type Volunteer, type VolunteerInput } from '@/lib/volunteers'
import { persistVolunteerInputs, loadVolunteerInputs } from '@/lib/volunteer-storage'
...
function normalizeJsonEntries(value: unknown): VolunteerInput[] {
  ...
  return value
    ...
    .filter(
      (entry): entry is VolunteerInput =>
        entry !== null && REQUIRED_FIELDS.every((field) => entry[field]?.length),
    )
}
...
export function VolunteerManager() {
  ...
  const [, setVolunteers] = useState<Volunteer[]>(getVolunteers())

  ...
  const handleScan = useCallback(
    async (decoded: string) => {
      ...
      try {
        ...
        // Fallback: mark entry/exit locally so UI updates even if API unreachable
        try {
          const local = loadLocalStatus(volunteerId)
          const updated = {
            ...local,
            entryVerified: mode === 'entry' ? true : local.entryVerified,
            exitVerified: mode === 'exit' ? true : local.exitVerified,
          }
